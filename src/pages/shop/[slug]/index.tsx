...
const index: React.FC<any> = ({ deviceType, initialProducts, colors, brands }) => {
  const [selectedBrandValues, setselectedBrandValues] = useState<any>([])
  const selectedBrandsForDataFetching = selectedBrandValues && selectedBrandValues.join("&company=");
...
`;
  ...
  const onchangeBrandHandler = (id: any, e:any) => {
    e.preventDefault(); 
  ...
  useEffect(() => {
    if (selectedBrandValues.length > 0) {
      ...
      if (selectedColor) {
        setFetchQuery("&company=" + selectedBrandsForDataFetching + "&colors__color_code=" + selectedColor.substring(1));
      }
      if (available) {
        setFetchQuery("&company=" + selectedBrandsForDataFetching + "&only_available=true")
      }
      if (orderby) {
        setFetchQuery("&company=" + selectedBrandsForDataFetching + "&ordering=" + orderby)
      }
    } else if (selectedColor) {
      setFetchQuery("&colors__color_code=" + '%23' + selectedColor.substring(1));
    } else if (available) {
                  ...
                  brands.map((brand: any, index: number) => (
                    <div className="d-flex flex-column mt-3">
                      ...
                      >
                        ...
                        <input
                          onChange={(e) => onchangeBrandHandler(brand.id, e)}
                          type="checkbox"
                          id={`cat-${index + 1}`}
                          style={{ cursor: "pointer" }}
                        />
      ...
      <Modal>
          ...
          >
            ...
            <Dropdown style={{ direction: "rtl" }} className="mr-3">
              ...
              <Dropdown.Menu style={{textAlign: 'center'}}>
                <Dropdown.Item onClick={() => setOrderby('-price')} >بیشترین قیمت</Dropdown.Item>
                <Dropdown.Item onClick={() => setOrderby('price')}>کمترین قیمت</Dropdown.Item>
                <Dropdown.Item onClick={() => setOrderby('sale_count')}>پر فروش ترین</Dropdown.Item>
              </Dropdown.Menu>
            </Dropdown>
          </div>
        </MobileCarouselDropdown>
        ...
        <MainContentArea>
          ...
          <SidebarSection>
            ...
            >
              <input id="s2" type="checkbox" className="switch" checked={available} onChange={() => setAvailable(!available)} />
              <label htmlFor="s2">فقط کالاهای موجود</label>
            </div>
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  const fetchInitialProducts = async () => {
    try {
      const res = await jwtAxios.get("products/?limit=6&offset=0");
      return res;
    } catch (error) {
  ...
  let brandsRes;
  let resColors;
  if (params && params.slug === "all") {
    res = await fetchInitialProducts();
    brandsRes = await fetchInitialBrands();
    resColors = await fetchInitialColors();
  } else if (params && params.slug === "trend") {
    res = await fetchInitialProducts();
    brandsRes = await fetchInitialBrands();
    resColors = await fetchInitialColors();
  }
