...
import { fetchSSGBrands, fetchSSGProducts } from "src/helpers";
...
`;
  ...
  if (products && products.length > 0) {
    ProductsContent = (
      <div ref={targetRef}>
        <Products deviceType={deviceType} fetchLimit={20} products={products} />
      </div>
    );
  }
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  let brandID = params && params.BID;
  const fetchProductsByBrand = async () => {
    try {
      console.log("fetching initil products started");
      const res = await jwtAxios.get(
        `products/?limit=50&offset=0&company=${brandID}`
      );
  ...
  let res = await fetchProductsByBrand();
...
export async function getStaticPaths() {
  let data = await fetchSSGBrands();

  // Get the paths we want to pre-render based on posts
  const paths = data.map((brand: any) => ({
      params: {BID: brand.id.toString()},
  }));
