...
import { fetchSSGCategories, fetchSSGProducts } from "src/helpers";
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  let categoryID = params && params.CAT;
  ...
  let res = await fetchProductsByCategory();
  if (!res.data) {
    return {
      redirect: {
        destination: "/",
...
export async function getStaticPaths() {
  let data = await fetchSSGCategories();

  // Get the paths we want to pre-render based on posts
  const paths = data.map((category: any) => ({
      params: {CAT: category.id.toString()},
  }));
